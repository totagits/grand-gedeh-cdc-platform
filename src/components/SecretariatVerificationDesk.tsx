...
export const SecretariatVerificationDesk: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'businesses' | 'workforce' | 'concessions' | 'audit_log'>('businesses');
  ...
  return (
    <section className="py-12 bg-slate-50 text-slate-800 dark:bg-slate-950 dark:text-slate-100 min-h-[90vh] transition-colors">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        ...
        {/* Sovereign Security Audit Header */}
        <div className="bg-gradient-to-br from-emerald-950 via-[#0d2a24] to-slate-950 text-white p-6 md:p-8 rounded-3xl shadow-xl border border-emerald-800/40 relative overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            <div className="max-w-2xl">
              <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-amber-500/20 border border-amber-400/40 text-amber-300 text-xs font-bold uppercase tracking-wider mb-3 backdrop-blur-md">
                <ShieldCheck className="w-3.5 h-3.5 text-amber-400" />
                <span>Executive Secretariat Legal &amp; Audit Authority</span>
              </div>
              ...
              <p className="text-emerald-100/85 text-xs sm:text-sm md:text-base leading-relaxed">
                Used by the GGCDC Secretariat to verify enterprise filings, inspect uploaded LBR/LRA tax clearances, authenticate qualifications, and award official local content accreditation for concession procurement (Putu, Singbeh, MPW).
              </p>
            </div>

            {/* Action Buttons & Sovereign Security Badge */}
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 shrink-0">
              <button
                type="button"
                onClick={() => setIsConcessionModalOpen(true)}
                className="px-4 py-3 rounded-2xl bg-gradient-to-r from-amber-500 to-amber-400 hover:from-amber-400 hover:to-amber-300 text-slate-950 font-black text-xs flex items-center justify-center space-x-2 shadow-lg transition-transform hover:scale-105 active:scale-95"
              >
                <Building2 className="w-4 h-4 text-slate-950" />
                <span>+ Ingest New Concession / Treaty</span>
              </button>

              <div className="bg-slate-900/80 border border-amber-500/40 rounded-2xl p-4 backdrop-blur-md">
                <div className="text-[10px] text-amber-400 font-bold uppercase tracking-wider">Audit Security Clearance</div>
                <div className="text-base font-black text-white font-mono mt-0.5">Section 13 Compliance</div>
                <div className="text-[11px] text-emerald-400 flex items-center gap-1.5 mt-1">
                  <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span>
                  <span>Dual GSM SMS &amp; SMTP Active</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3.5">
          <div className="bg-white/95 dark:bg-slate-900/95 border border-slate-200/90 dark:border-slate-800/90 rounded-2xl p-4 shadow-sm">
            <span className="text-[11px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider block mb-1">Enterprises</span>
            <span className="text-2xl font-black text-slate-900 dark:text-white font-mono">{businesses.length}</span>
          ...
          <div className="bg-white/95 dark:bg-slate-900/95 border border-slate-200/90 dark:border-slate-800/90 rounded-2xl p-4 shadow-sm">
            <span className="text-[11px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider block mb-1">Pending Business</span>
            <span className="text-2xl font-black text-amber-600 dark:text-amber-400 font-mono">
              {businesses.filter(b => b.verificationStatus === 'Pending Secretarial Audit').length}
            </span>
            <span className="text-[11px] text-slate-500 dark:text-slate-400 block mt-1">LBR Check Queued</span>
          </div>

          <div className="bg-white/95 dark:bg-slate-900/95 border border-slate-200/90 dark:border-slate-800/90 rounded-2xl p-4 shadow-sm">
            <span className="text-[11px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider block mb-1">Talent Roster</span>
            <span className="text-2xl font-black text-slate-900 dark:text-white font-mono">{workforce.length}</span>
            <span className="text-[11px] text-emerald-700 dark:text-emerald-400 font-medium block mt-1">
          ...
          <div className="bg-white/95 dark:bg-slate-900/95 border border-slate-200/90 dark:border-slate-800/90 rounded-2xl p-4 shadow-sm">
            <span className="text-[11px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider block mb-1">Pending Talent</span>
            <span className="text-2xl font-black text-amber-600 dark:text-amber-400 font-mono">
              {workforce.filter(w => w.verificationStatus === 'Pending Secretarial Audit').length}
            </span>
            <span className="text-[11px] text-slate-500 dark:text-slate-400 block mt-1">TVET Check Queued</span>
          </div>

          <div className="bg-white/95 dark:bg-slate-900/95 border border-slate-200/90 dark:border-slate-800/90 rounded-2xl p-4 col-span-2 sm:col-span-1 shadow-sm">
            <span className="text-[11px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider block mb-1">Disqualified</span>
            ...
            <span className="text-[11px] text-rose-600/90 dark:text-rose-400/90 block mt-1">14-Day Appeal Open</span>
          </div>
        </div>

        {/* Action Success Alert */}
        {actionSuccessNotice && (
          <div className="p-4 bg-emerald-50 dark:bg-emerald-950/90 border border-emerald-300 dark:border-emerald-500 rounded-2xl text-xs text-emerald-900 dark:text-emerald-200 flex items-center space-x-3 shadow-md animate-fadeIn">
        ...
        {/* Primary Tabs */}
        <div className="flex flex-wrap items-center gap-2 sm:gap-3 border-b border-slate-200 dark:border-slate-800 pb-3">
          <button
          ...
          >
            <Building2 className="w-4 h-4" />
            <span>Concession Intake &amp; Treaties ({concessions.length})</span>
          </button>
        ...
        {/* Content: Concession Treaties & Statutory Intake Desk */}
        {activeTab === 'concessions' && (
          <div className="space-y-6">
            {/* Header Banner */}
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-8 shadow-sm dark:shadow-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
              <div className="max-w-2xl">
                <div className="inline-flex items-center space-x-2 text-xs font-bold text-emerald-800 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/80 border border-emerald-300 dark:border-emerald-800 px-3 py-1 rounded-full uppercase tracking-wider mb-2.5">
                  <Building2 className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" />
                  <span>Statutory Treaty Register &amp; Ingestion Desk</span>
                </div>
                <h3 className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white tracking-tight">
                  Ratified Concession Agreements &amp; Commitments
                </h3>
                <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-300 mt-1.5 leading-relaxed">
                  Every official concession operating in Grand Gedeh is gazetted here with its statutory counterpart (MME, FDA, NBC), legislative ratification act, annual County Social Development Fund (CSDF) covenants, and mandatory Section 11/13 local content quotas.
                </p>
              </div>
              ...
              <button
                type="button"
                onClick={() => setIsConcessionModalOpen(true)}
                className="px-5 py-3 rounded-2xl bg-gradient-to-r from-amber-500 to-amber-400 hover:from-amber-400 hover:to-amber-300 text-slate-950 font-black text-xs flex items-center space-x-2 shadow-lg transition-transform hover:scale-105 active:scale-95 shrink-0"
              >
                <Building2 className="w-4 h-4 text-slate-950" />
                <span>+ Ingest New Concession / Treaty</span>
              </button>
            ...
            {/* Concession Treaties Grid */}
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {concessions.map((p) => (
                <div 
                  key={p.id}
                  className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-5 sm:p-6 shadow-sm hover:shadow-md transition-all flex flex-col justify-between"
                >
                  <div className="space-y-3">
                    <div className="flex items-center justify-between gap-2">
                      <span className="text-[10px] uppercase font-black px-2.5 py-0.5 rounded-full bg-emerald-50 dark:bg-emerald-950/80 text-emerald-800 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800">
                        {p.sector}
                      </span>
                    ...
                    <div className="text-xs text-slate-600 dark:text-slate-400 space-y-1">
                      <div><strong className="text-slate-800 dark:text-slate-200">Operator:</strong> {p.operator}</div>
                      <div><strong className="text-slate-800 dark:text-slate-200">Investor Origin:</strong> {p.investorOrigin}</div>
                      <div><strong className="text-slate-800 dark:text-slate-200">District:</strong> {p.district}</div>
                      <div><strong className="text-slate-800 dark:text-slate-200">Statutory Counterpart:</strong> {p.govCounterpart}</div>
                    </div>

                    <div className="pt-2 border-t border-slate-100 dark:border-slate-800 space-y-1 text-xs">
                      <div className="flex items-center justify-between text-slate-600 dark:text-slate-400">
                        <span>CSDF Escrow:</span>
                        <span className="font-bold text-emerald-700 dark:text-emerald-400 font-mono">
                          {p.commitmentsSummary.socialDevFundAnnual.split('(')[0]}
                        </span>
                      </div>
                      <div className="flex items-center justify-between text-slate-600 dark:text-slate-400">
                        <span>Local Hiring Quota:</span>
                        <span className="font-semibold text-slate-800 dark:text-slate-200">
                          {p.commitmentsSummary.employmentGoal.split(' ')[0]}
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="mt-5 pt-3 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between gap-2">
                    <button
                      ...
                      className="px-3 py-1.5 rounded-lg bg-emerald-50 hover:bg-emerald-100 dark:bg-emerald-950/80 dark:hover:bg-emerald-900/90 text-emerald-800 dark:text-emerald-300 font-bold text-xs transition-colors"
                    >
                      Inspect Profile →
                    </button>
