    ...
    <section className="py-14 bg-slate-900 border-b border-slate-800">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12">
          <div className="inline-flex items-center space-x-2 bg-emerald-950/80 text-emerald-400 border border-emerald-800/80 px-3.5 py-1 rounded-full text-xs font-semibold uppercase tracking-wider mb-3">
            ...
            return (
              <div
                ...
                className={`cursor-pointer rounded-xl p-5 border transition-all duration-200 text-left relative overflow-hidden flex flex-col justify-between ${
                  isSelected
                    ? 'bg-gradient-to-br from-emerald-950 via-slate-800 to-slate-900 border-emerald-500 shadow-xl ring-2 ring-emerald-500/30'
              ...
              >
                ...
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <div className={`p-2.5 rounded-lg ${isSelected ? 'bg-emerald-600 text-white' : 'bg-slate-700 text-emerald-400'}`}>
                  ...
                  <h3 className="font-bold text-white text-base leading-snug group-hover:text-emerald-400">
                    {pillar.name}
                  </h3>
                  <div className="text-[11px] text-amber-400/90 font-medium mt-0.5 line-clamp-1">
                    {pillar.directorate}
                  </div>
                ...
                <div className="mt-4 pt-3 border-t border-slate-700/60 flex items-center justify-between text-xs">
                  ...
                  <span className={`text-[11px] font-semibold flex items-center ${isSelected ? 'text-emerald-400' : 'text-slate-400'}`}>
                    <span>Explore</span>
        ...
        <div className="bg-slate-800/90 border border-emerald-800/60 rounded-2xl p-6 sm:p-8 shadow-2xl backdrop-blur-md">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 pb-6 border-b border-slate-700">
            <div>
              <div className="flex items-center space-x-2 text-xs font-semibold text-emerald-400 uppercase tracking-wider mb-1">
                ...
                <span>•</span>
                <span className="text-amber-400">{selectedPillar.directorate}</span>
              </div>
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <button
                ...
                className="bg-slate-700 hover:bg-slate-600 text-white text-xs font-semibold px-4 py-2.5 rounded-lg flex items-center space-x-1.5"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pt-6">
            ...
            <div className="bg-slate-900/70 border border-slate-700/80 rounded-xl p-5">
              <h4 className="text-sm font-bold text-emerald-300 uppercase tracking-wider mb-3 flex items-center space-x-2">
                ...
                {selectedPillar.operationalTools.map((tool, idx) => (
                  <li key={idx}>
                    <button
                      ...
                      className="w-full text-left flex items-start space-x-2.5 text-xs text-slate-200 hover:text-emerald-300 hover:bg-slate-800 p-2 rounded transition-colors group"
                    >
                      <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500 mt-0.5 flex-shrink-0 group-hover:scale-110 transition-transform" />
            ...
            <div className="bg-slate-900/70 border border-slate-700/80 rounded-xl p-5">
              <h4 className="text-sm font-bold text-amber-300 uppercase tracking-wider mb-3 flex items-center space-x-2">
              ...
              <div className="space-y-3">
                {selectedPillar.keyInitiatives.map((init, idx) => (
                  <div key={idx} className="bg-slate-800/90 border border-slate-700 p-3 rounded-lg">
                    <div className="text-xs font-semibold text-white">{init}</div>
                    <div className="flex items-center justify-between mt-2 pt-2 border-t border-slate-700/60 text-[11px] text-slate-400">
                      <span className="text-emerald-400 font-mono">Status: Active Supervision</span>
                      <button 
            ...
            <div className="bg-slate-900/70 border border-slate-700/80 rounded-xl p-5 flex flex-col justify-between">
              <div>
                <h4 className="text-sm font-bold text-blue-300 uppercase tracking-wider mb-3 flex items-center space-x-2">
                  <Building2 className="w-4 h-4 text-blue-400" />
                  <span>Directorate Baseline</span>
                </h4>
                <div className="bg-slate-800/90 border border-slate-700 p-3.5 rounded-lg mb-4">
                  <div className="text-xs text-slate-400">Coverage & Scale:</div>
                  <div className="text-sm font-bold text-white mt-1">{selectedPillar.stats}</div>
                </div>
              ...
              <div className="pt-4 border-t border-slate-700/80">
                <button
                  ...
                  className="w-full text-center bg-slate-800 hover:bg-emerald-900/60 text-emerald-300 hover:text-white border border-emerald-700/60 text-xs font-bold py-2.5 rounded-lg transition-colors"
                >
