    ...
    <section className="py-12 bg-slate-950 text-slate-100 min-h-[85vh]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          ...
          <div className="bg-slate-900 border border-slate-800 p-1.5 rounded-xl flex items-center space-x-2 self-start md:self-auto">
            <button
        ...
        <div className="flex items-center space-x-2 overflow-x-auto pb-3 mb-8 border-b border-slate-800">
            ...
            return (
              <button
                ...
                className={`px-4 py-2 rounded-xl text-xs font-semibold whitespace-nowrap transition-all flex items-center space-x-2 ${
                  isSelected
                    ? 'bg-slate-800 text-emerald-400 border border-emerald-600/80 shadow-lg'
                    : 'bg-slate-900/60 text-slate-400 hover:bg-slate-900 hover:text-slate-200 border border-slate-800'
                }`}
              >
                <span>{group.name}</span>
                {group.id === 'wg-putu' && (
                  <span className="bg-amber-500/20 text-amber-300 text-[10px] px-1.5 py-0.2 rounded font-bold border border-amber-500/40">
                    Flagship
                  </span>
                )}
              </button>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 sm:p-8 mb-6 shadow-2xl">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 pb-6 border-b border-slate-800">
            <div>
              <div className="flex items-center space-x-2 text-xs font-bold text-amber-400 uppercase tracking-wider mb-1">
                ...
                <span>•</span>
                <span className="text-emerald-400">{activeGroup.memberCount} Working Group Members</span>
              </div>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
              {activeGroup.id === 'wg-putu' && (
                <button
                  ...
                  className="bg-amber-600 hover:bg-amber-500 text-slate-950 font-bold text-xs px-4 py-2 rounded-lg flex items-center space-x-1.5 shadow"
                >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 pt-6 text-xs">
            <div className="bg-slate-950 p-3 rounded-xl border border-slate-800">
              <span className="text-slate-500 block text-[10px] uppercase font-semibold">Lead Coordinator:</span>
              <span className="font-bold text-white mt-0.5 block">{activeGroup.leadCoordinator}</span>
            </div>
            <div className="bg-slate-950 p-3 rounded-xl border border-slate-800">
              <span className="text-slate-500 block text-[10px] uppercase font-semibold">Deputy Lead:</span>
              <span className="font-bold text-white mt-0.5 block">{activeGroup.deputyLead}</span>
            </div>
            <div className="bg-slate-950 p-3 rounded-xl border border-slate-800">
              <span className="text-slate-500 block text-[10px] uppercase font-semibold">Active Monitored Projects:</span>
              <span className="font-bold text-emerald-400 mt-0.5 block">{activeGroup.activeProjects.join(', ')}</span>
            </div>
            <div className="bg-slate-950 p-3 rounded-xl border border-slate-800">
              <span className="text-slate-500 block text-[10px] uppercase font-semibold">Published Reports:</span>
              <span className="font-bold text-amber-300 mt-0.5 block">{activeGroup.publicReportsCount} Public Briefs</span>
            </div>
          </div>
        </div>
        ...
        {workspaceMode === 'secure' ? (
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 sm:p-8 shadow-2xl">
            ...
            {!isAuthorized && (
              <div className="mb-6 p-4 bg-amber-950/40 border border-amber-500/50 rounded-xl text-xs text-amber-200 flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  <Lock className="w-5 h-5 text-amber-400 flex-shrink-0" />
                  <div>
                    <span className="font-bold text-white block">Previewing Institutional Operational Workspace</span>
                ...
                <span className="bg-amber-500 text-slate-950 font-bold px-2.5 py-1 rounded text-[11px]">
                  Read-Only Mode
                </span>
              </div>
            )}
            ...
            {secureTab === 'tasks' && (
              <div className="space-y-3">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs text-slate-400">Current assigned tasks for this working group</span>
                  <button className="bg-slate-800 hover:bg-slate-700 text-emerald-400 border border-slate-700 text-xs px-3 py-1.5 rounded-lg flex items-center space-x-1">
                ...
                {activeGroup.tasks.map((task) => (
                  <div key={task.id} className="p-4 bg-slate-950/80 border border-slate-800 rounded-xl flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
                    <div className="space-y-1">
                      <div className="flex items-center space-x-2">
                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded ${
                          task.priority === 'Critical' ? 'bg-red-950 text-red-300 border border-red-800' : 'bg-amber-950 text-amber-300 border border-amber-800'
                        }`}>
                          {task.priority} Priority
                        </span>
                        <span className="font-mono text-slate-500">{task.id.toUpperCase()}</span>
                      </div>
                      <h4 className="font-bold text-white text-sm">{task.title}</h4>
                      <div className="text-slate-400">
                        Assigned To: <span className="text-slate-200">{task.assignedTo}</span> • Due: <span className="text-amber-300 font-mono">{task.dueDate}</span>
                      </div>
                    </div>

                    <span className="bg-slate-800 text-slate-300 px-3 py-1 rounded-full text-xs font-semibold self-start sm:self-auto">
                      {task.status}
                    </span>
                  </div>
                ))}
            ...
            {secureTab === 'risks' && (
              <div className="space-y-3">
                {activeGroup.risks.map((rsk) => (
                  <div key={rsk.id} className="p-4 bg-slate-950/80 border border-slate-800 rounded-xl text-xs space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="text-red-400 font-bold bg-red-950/80 border border-red-800 px-2 py-0.5 rounded uppercase text-[10px]">
                        Severity: {rsk.severity}
                      </span>
                      <span className="font-mono text-slate-500">{rsk.id.toUpperCase()}</span>
                    </div>
                    <div className="text-white font-bold text-sm">{rsk.risk}</div>
                    <div className="p-2.5 bg-slate-900 rounded-lg border border-slate-800 text-slate-300">
                      <span className="text-emerald-400 font-semibold">Agreed Working Group Mitigation Strategy: </span>
                      {rsk.mitigation}
                    </div>
                  </div>
                ))}
            ...
            {secureTab === 'legalNotes' && (
              <div className="p-5 bg-slate-950/80 border border-slate-800 rounded-xl text-xs space-y-3">
                <div className="flex items-center justify-between pb-3 border-b border-slate-800">
                  <span className="font-bold text-white text-sm">Working Group Legal & Technical Analysis Memo</span>
                  <span className="text-amber-400 font-mono text-[11px]">Confidential Internal</span>
                </div>
            ...
            {secureTab === 'minutes' && (
              <div className="p-5 bg-slate-950/80 border border-slate-800 rounded-xl text-xs space-y-3">
                <div className="flex items-center justify-between pb-3 border-b border-slate-800">
                  <span className="font-bold text-white text-sm">Latest Formal Session Summary</span>
                  <span className="text-emerald-400 font-mono text-[11px]">September 2026 Session</span>
                </div>
                <p className="text-slate-300 leading-relaxed">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 sm:p-8 shadow-2xl space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
              <div className="bg-slate-950 p-4 rounded-xl border border-slate-800">
                <h5 className="font-bold text-white mb-2">Public Documentation Released</h5>
              ...
              <div className="bg-slate-950 p-4 rounded-xl border border-slate-800">
                <h5 className="font-bold text-white mb-2">Citizen Participation Channels</h5>
                <ul className="space-y-1.5 text-slate-300">
