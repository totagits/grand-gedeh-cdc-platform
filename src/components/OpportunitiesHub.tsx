        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          ...
          <form onSubmit={handleSubscribe} className="flex items-center space-x-2 self-start md:self-auto">
            <input
              ...
              className="bg-white dark:bg-slate-950 border border-slate-300 dark:border-slate-700 text-xs px-3 py-2 rounded-lg text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:border-emerald-500 shadow-sm"
            />
            <button
              type="submit"
            ...
            >
              <div>
                ...
                <div className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                  Issued by: <span className="font-semibold text-slate-700 dark:text-slate-300">{opp.issuer}</span>
                </div>

                <div className="mt-3 p-3 bg-slate-50 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 rounded-lg text-xs space-y-1.5">
                  <div className="flex items-center justify-between">
                    <span className="text-slate-500 dark:text-slate-400">Sector:</span>
                    <span className="text-slate-900 dark:text-white font-medium">{opp.sector}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-slate-500 dark:text-slate-400">Location:</span>
                    <span className="text-slate-700 dark:text-slate-300">{opp.location}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-slate-500 dark:text-slate-400">Value / Scope:</span>
                    <span className="text-emerald-700 dark:text-emerald-400 font-bold">{opp.compensationOrValue}</span>
                  </div>
                </div>
              ...
              {/* Action Button */}
              <div className="mt-5 pt-3 border-t border-slate-100 dark:border-slate-700/80 flex items-center justify-between">
                <span className="text-[10px] text-slate-400 font-mono">ID: {opp.id.toUpperCase()}</span>
                <button className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs px-3.5 py-1.5 rounded-lg transition-colors shadow-sm">
